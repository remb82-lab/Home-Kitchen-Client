...
(function(){
  ...
  function escText(value){return String(value==null?'':value).replace(/[&<>"']/g,function(ch){return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'})[ch];});}
  function statusText(status){
    var known={new:'Заказ принят',pending:'Заказ принят',confirmed:'Заказ подтверждён',needs_production:'Готовим ваш заказ',preparing:'Готовим ваш заказ',ready:'Заказ готов к выдаче',issued:'Заказ выдан',cancelled:'Заказ отменён'};
  ...
  async function latestOrder(){if(typeof orderRefs==='undefined'||!Array.isArray(orderRefs)||!orderRefs.length) return null;var fallback=null;for(var i=0;i<Math.min(orderRefs.length,5);i++){var ref=orderRefs[i];var order=await fetchOrder(ref);if(!order) continue;var entry={ref:ref,order:order,index:i};if(!fallback) fallback=entry;if(ACTIVE_STATUSES.has(String(order.status||''))) return entry;}return fallback;}
  function currentOrderHost(){var catalog=document.getElementById('catalog');if(!catalog) return null;var existing=document.getElementById('hk-current-order-card');if(existing) return existing;var card=document.createElement('section');card.id='hk-current-order-card';card.className='hk-current-order-card';card.hidden=true;var sticky=catalog.querySelector('.sticky');if(sticky) sticky.insertAdjacentElement('afterend',card);else{var header=catalog.querySelector('header');if(header) header.insertAdjacentElement('afterend',card);else catalog.prepend(card);}return card;}
  async function renderCurrentOrder(){var host=currentOrderHost();if(!host) return;var latest=await latestOrder();if(!latest||!latest.order){host.hidden=true;host.innerHTML='';return;}var order=latest.order;var active=ACTIVE_STATUSES.has(String(order.status||''));host.hidden=false;host.dataset.status=String(order.status||'');host.innerHTML='<div class="hk-current-order-card__head"><div><span class="hk-current-order-card__eyebrow">'+(active?'ТЕКУЩИЙ ЗАКАЗ':'ПОСЛЕДНИЙ ЗАКАЗ')+'</span><strong>Заказ '+escText(order.id||'')+'</strong></div><span class="hk-current-order-card__status">'+escText(statusText(order.status))+'</span></div><div class="hk-current-order-card__meta">'+(order.total_amount!=null?'<span>Итого: <b>'+escText(fmtMoney(order.total_amount))+'</b></span>':'')+'<span>Позиции: <b>'+String(Array.isArray(order.items)?order.items.length:0)+'</b></span></div><div class="hk-current-order-card__actions"><button type="button" class="secondary" data-hk-open-orders>Открыть заказ</button><button type="button" class="primary" data-hk-repeat-order="'+String(latest.index)+'">Заказать снова</button></div>';}
