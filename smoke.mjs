...
const card=await read('catalog/product-card.js');
const detail=await read('catalog/product-detail.js');
const cartView=await read('cart/cart-view.js');
...
if(html.includes('id="pbackend"')||html.includes('Supabase Cloud')||html.includes('Подключение к Supabase'))throw new Error('Technical backend details must not be customer-visible');

for(const token of ['/functions/v1/create-client-order','turnstile_token'])if(!api.includes(token))throw new Error('API boundary missing '+token);
for(const token of ['0x4AAAAAAEvNfqWeNAvsAKeB',"appearance:'interaction-only'","execution:'execute'","api.execute('#'+HOST_ID)"])if(!turnstile.includes(token))throw new Error('Turnstile guard missing '+token);
...
if(!card.includes('price_kg')||card.includes('price_half'))throw new Error('Catalog card must show only the per-kg price');
if(!detail.includes('price_kg')||detail.includes('price_half')||detail.includes('add-half'))throw new Error('Product detail must use one per-kg price and separate weight choice');
...
if(!sw.includes("`${CACHE_PREFIX}v16`"))throw new Error('PWA cache was not rotated to v16');
...
const requiredFiles=['runtime/client-runtime.js','services/client-api.js','security/turnstile.js','contact/client-contact.js','catalog/catalog-data.js','catalog/product-card.js','catalog/product-detail.js','catalog/catalog-controller.js','catalog/product-share.js','cart/cart-view.js','cart/cart-controller.js','checkout/checkout-view.js','checkout/checkout-controller.js','orders/orders-view.js','orders/orders-controller.js','profile/profile-view.js','profile/profile-controller.js','app/app-state.js','app/navigation-controller.js','app/bootstrap.js','client-repeat-order-adapter.js','assets/branding/home-kitchen-mark.svg'];
...
const product50=catalog.find((item)=>Number(item.id)===50);
if(!product50)throw new Error('Product 50 missing from catalog');
const image=String(product50.image_url||'').trim();
if(!image)throw new Error('Product 50 must have its approved photo');
if(image.startsWith('data:image/')){
  if(!/^data:image\/webp;base64,[A-Za-z0-9+/=]+$/.test(image))throw new Error('Product 50 embedded image is invalid');
}else if(/^https?:\/\//.test(image)){
  const imageResponse=await fetch(image,{method:'GET'});
  if(!imageResponse.ok)throw new Error('Product 50 remote photo is unavailable: '+imageResponse.status);
}else{
  await fs.access(image.replace(/^\.\//,''));
}

console.log('PASS: Turnstile mobile + customer UX + product 50 photo + live catalog; items='+catalog.length);
