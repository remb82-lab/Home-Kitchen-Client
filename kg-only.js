...
(function(){
  ...
  function installKgOnlyMode(){
    ...
    renderCatalog=function(){
      ...
      E.grid.innerHTML=arr.length?arr.map(function(p){return `<article class="product">${image(p)}<div class="pbody"><span class="badge">${esc(p.assortment_status||'В продаже')}</span><div class="ptitle" onclick="openDetail('${p.id}')">${esc(p.name)}</div><div class="prices"><div class="price"><span>1 кг</span><b>${fmt(p.price_kg)}</b></div></div><div class="freeze">❄️ Замороженный продукт</div><div class="actions"><button class="secondary" onclick="openDetail('${p.id}')">Подробнее</button><button class="primary" onclick="add('${p.id}',1000)">В корзину</button></div></div></article>`;}).join(''):'<div class="empty" style="grid-column:1/-1">Сейчас нет опубликованных позиций</div>';
    };

    openDetail=function(id){
      var p=getP(id);if(!p)return;
      E.detailContent.innerHTML=`<div class="top"><button class="back" onclick="go('catalog')">‹</button><h3>Карточка продукта</h3><span></span></div>${image(p,true)}<div class="detail"><span class="badge">${esc(p.assortment_status||'В продаже')}</span><h1>${esc(p.name)}</h1><div class="desc">${esc(p.description||'Домашний замороженный полуфабрикат.')}</div><div class="dprices" style="grid-template-columns:1fr"><div class="dprice"><span>1 кг</span><b>${fmt(p.price_kg)}</b><button class="checkout" onclick="add('${p.id}',1000)">Добавить 1 кг</button></div></div><div class="info"><b>Как приготовить</b><br>${esc(p.instruction||p.client_instruction||'Готовить по инструкции хозяйки.')}</div><div class="info">❄️ Продукт передаётся в замороженном виде.</div><div class="detail-actions"><button class="secondary" onclick="shareProduct('${p.id}')">Поделиться</button><button class="primary" onclick="openCart()">Корзина</button></div></div>`;
      go('detail');
    };
